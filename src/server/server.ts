...
import {
  ...
  type IncCounterReq,
  type IncCounterRsp,
  type SearchStoriesReq,
  type SearchStoriesRsp,
} from '../shared/api.ts'
  ...
  if (method !== reqMsg.method) {
    ...
    switch (endpoint) {
      ...
      case Endpoint.IncCounter:
        ...
        break
      case Endpoint.SearchStories:
        rsp = await routeSearchStories(reqMsg)
        break
...
async function routeSearchStories(
  reqMsg: IncomingMessage,
): Promise<SearchStoriesRsp> {
  const req = await readJson<SearchStoriesReq>(reqMsg)
  const term = req.term?.trim()
  const subreddit = req.subreddit?.replace(/^r\//, '').trim()
  if (!term || !subreddit) throw Error('term e subreddit são obrigatórios')
  ...
  return {
    stories: posts.map(post => ({
      ...
      title: post.title,
      author: post.authorName ? `u/${post.authorName}` : 'autor deletado',
      excerpt:
        post.body?.slice(0, 240) ||
        'Esta publicação não possui texto disponível.',
      text: post.body || '',
